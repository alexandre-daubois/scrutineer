...
// repository, which is the whole point of importing them. A record that
// fails to decrypt, validate or decode is logged and skipped so one bad
// file from a peer does not cost the rest of the feed.
func (s *Server) importFeed(ctx context.Context, remote string, repoIDs map[string]uint) error {
	...
	for _, rec := range records {
		...
		changed, err := s.storeImportedRecord(remote, rec)
		if err != nil {
		...
		fresh++
		if err := s.applyImportedRecord(rec.Statement, repoIDs, remote); err != nil {
			s.Log.Error("federation: apply record", "remote", remote, "path", rec.Path, "err", err)
		}
	}
	s.Log.Info("federation: imported feed", "remote", remote, "stored", stored, "changed", fresh, "skipped", failed)
	return nil
}

// storeImportedRecord archives the peer's own bytes and reports whether
// they actually changed. Only a changed record is applied locally: an
// unchanged one has already had its effect, and re-applying it every hour
// would silently undo an operator who cleared what a peer once asked for.
func (s *Server) storeImportedRecord(remote string, rec interchange.FeedRecord) (bool, error) {
	key := db.InterchangeRecord{
		Feed:          remote,
	...
	res := s.DB.Where(&key).Limit(1).Find(&existing)
	if res.Error != nil {
	...
	if res.RowsAffected == 1 {
		if existing.Record == raw {
			return false, s.DB.Model(&existing).UpdateColumn("received_at", now).Error
		}
		return true, s.DB.Model(&existing).Updates(map[string]any{"record": raw, "received_at": now}).Error
	}
	row := key
	row.Record = raw
	row.ReceivedAt = now
...
func (s *Server) applyImportedRecord(rec interchange.Statement, repoIDs map[string]uint, feed string) error {
	...
	case interchange.PredicateTypeOptOut:
		...
		repo, ok, err := s.repoForRecord(repoIDs, p.Repository)
		if err != nil || !ok || repo.FederationOptedOut() {
			return err
		}
		at := p.RequestedAt.UTC()
		return s.DB.Model(&db.Repository{}).Where("id = ?", repo.ID).Updates(map[string]any{
	...
	case interchange.PredicateTypeRoute:
		...
		repo, ok, err := s.repoForRecord(repoIDs, p.Repository)
		if err != nil || !ok || repo.DisclosureChannel != "" || repo.FederationOptedOut() {
			return err
		}
		at := p.VerifiedAt.UTC()
...
func (s *Server) repoForRecord(repoIDs map[string]uint, url string) (db.Repository, bool, error) {
...
func runFeedGit(ctx context.Context, dir string, args ...string) (string, error) {
	return gitx.Retry{}.Do(ctx, gitx.Command{
		Label: "feed " + args[0],
		Dir:   dir,
